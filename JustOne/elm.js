...
var $author$project$HomePage$Error = function (a) {
	return {$: 'Error', a: a};
};
var $author$project$HomePage$ModelLoaded = function (a) {
	return {$: 'ModelLoaded', a: a};
};
...
var $author$project$HomePage$Term = F2(
	function (term, category1) {
		return {category1: category1, term: term};
	});
...
var $author$project$HomePage$termDecoder = A2(
	$BrianHicks$elm_csv$Csv$Decode$pipeline,
	A2($BrianHicks$elm_csv$Csv$Decode$field, 'Kategorie 1', $BrianHicks$elm_csv$Csv$Decode$string),
	A2(
		$BrianHicks$elm_csv$Csv$Decode$pipeline,
		A2($BrianHicks$elm_csv$Csv$Decode$field, 'Begriff', $BrianHicks$elm_csv$Csv$Decode$string),
		$BrianHicks$elm_csv$Csv$Decode$into($author$project$HomePage$Term)));
var $author$project$HomePage$decodeTerms = A2($BrianHicks$elm_csv$Csv$Decode$decodeCsv, $BrianHicks$elm_csv$Csv$Decode$FieldNamesFromFirstRow, $author$project$HomePage$termDecoder);
var $author$project$HomePage$parseTerm = function (arg1) {
	if (arg1.$ === 'Err') {
		var text = arg1.a;
		return $author$project$HomePage$Error('OOPS');
	} else {
		var result = arg1.a;
		var _v1 = $author$project$HomePage$decodeTerms(result);
		if (_v1.$ === 'Err') {
			var text = _v1.a;
			return $author$project$HomePage$Error('OOPS2');
		} else {
			var terms = _v1.a;
			return $author$project$HomePage$ModelLoaded(terms);
		}
	}
};
...
var $author$project$HomePage$getTerms = $elm$http$Http$get(
	{
		...
		url: $author$project$HomePage$url
	});
...
var $author$project$HomePage$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'LoadTerms':
				return _Utils_Tuple2($author$project$HomePage$emptyModel, $author$project$HomePage$getTerms);
			case 'ModelLoaded':
				var terms = msg.a;
				return _Utils_Tuple2(
					$author$project$HomePage$init(terms),
				...
				if (mTerm.$ === 'Nothing') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								error: $elm$core$Maybe$Just('No more terms')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var term = mTerm.a;
			...
			default:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							error: $elm$core$Maybe$Just(text)
						}),
					$elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$HomePage$colortext = F2(
...
var $author$project$HomePage$logo = A2(
	...
	_List_fromArray(
		[
			...
			A2($author$project$HomePage$colortext, 'red', 'U'),
			A2($author$project$HomePage$colortext, 'pink', 'S'),
			A2($author$project$HomePage$colortext, 'purple', 'T '),
			A2($author$project$HomePage$colortext, 'blue', 'O'),
			A2($author$project$HomePage$colortext, 'green', 'N'),
			A2($author$project$HomePage$colortext, '#D4AC0D', 'E')
		]));
var $elm$html$Html$hr = _VirtualDom_node('hr');
var $author$project$HomePage$viewSeparator = A2(
	$elm$html$Html$hr,
	_List_fromArray(
		[
	...
	_List_Nil);
var $author$project$HomePage$viewApp = F2(
...
var $author$project$HomePage$viewError = function (err) {
	return A2(
		$author$project$HomePage$viewApp,
		A2($author$project$HomePage$colortext, 'red', 'Fehler!'),
		_List_fromArray(
			[
				A2(
				$elm$html$Html$span,
...
var $author$project$HomePage$viewInit = A2(
	$author$project$HomePage$viewApp,
...
var $author$project$HomePage$viewTerms = function (gamestate) {
	var begriff = function () {
		var _v0 = gamestate.current;
		if (_v0.$ === 'Just') {
			var term = _v0.a;
...
var $author$project$HomePage$view = function (model) {
	...
	if (_v0.$ === 'Just') {
		var err = _v0.a;
		return $author$project$HomePage$viewError(err);
	} else {
		var _v1 = model.terms;
		if (!_v1.b) {
			return $author$project$HomePage$viewInit;
		} else {
			return $author$project$HomePage$viewTerms(model);
		}
	}
};
